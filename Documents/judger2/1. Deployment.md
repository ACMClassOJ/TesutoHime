...
在浏览器中打开 <http://10.0.0.1:9000>，会跳转到一个登录界面，
用刚才设置的密码登录。
...
```
https://acm.sjtu.edu.cn/oj-problems/* -> http://10.0.0.1:9000/oj-problems/*
https://acm.sjtu.edu.cn/oj-submissions/* -> http://10.0.0.1:9000/oj-submissions/*
https://acm.sjtu.edu.cn/oj-images/* -> http://10.0.0.1:9000/oj-images/*
...
http://10.0.0.1:9000，S3 public URL 填 <https://acm.sjtu.edu.cn>。
...
为了权限最小化，我们给评测机和调度机各分配一个 redis 用户。
...
```
AUTH 这里填默认用户密码
ACL SETUSER scheduler on >xxxxxxxx +get|lpush|blpop|expire|lrange
ACL SETUSER runner on >xxxxxxxx +set|delete|blmove|lpush|expire|lpop|blpop|lrem|blmove
```
...
编译所需的二进制文件: (会编译一遍 coreutils，耗时可能较长)

```sh
cd /path/to/TesutoHime/judger2/sandbox
make
cd /path/to/TesutoHime/steps
make
```
