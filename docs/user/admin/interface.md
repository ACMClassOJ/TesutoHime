...
- 只读权限：在发布时间之前查看题目；下载题目数据包；查看本题目下任意用户提交的代码及评测状态。
- 完整权限：对题目拥有完整权限。
...
作业、比赛和考试均有其所属的班级，只有所属班级的管理员才能对其进行管理。如果在班级内新建了作业、比赛或考试，那么对应的学生就会自动加入，无需学生主动加入。具体来说，如果没有设置分组，那么所有班级内的学生都会自动加入；如果设置了分组，那么只有被指定的分组的学生会自动加入。其他的学生（包括非指定分组和其他班级的学生）仍然可以通过报名加入。
