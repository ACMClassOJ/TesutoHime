...
导出的文件名为 `export-id.zip`，其中 `id` 为作业、比赛或考试的编号。在此压缩包内，每个题目的提交会分别放在以题号为名称的目录下，每个目录内的文件名为 `学工号-实名-用户名-提交编号-P题目编号-提交状态-得分.后缀`。在每个文件中，其头部会用注释的方式包含提交一些信息，包括用户、题目、时间、状态、得分、评测详细信息、每个评测点的情况等。
