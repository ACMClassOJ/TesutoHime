...
async def check(outfile: CheckInput, checker: Checker, checker_id: int, run_res_path: PosixPath) -> CheckResult:
    logger.info(f'checking with {checker}, checker_id {checker_id}')
...
async def checker_direct(_infile, outfile: PosixPath, run_res_path, _checker, _checker_id):
    return checker_read_float(outfile)


async def checker_spj(infile: Optional[PosixPath], outfile: PosixPath, \
    run_res_path: PosixPath, checker: SpjChecker, checker_id: int):
    # get spj binary
    ...
    with TempDir() as cwd:
        ...
        exec_file = cwd / f'spj_{str(checker_id)}'
        copy2(exe, exec_file)
